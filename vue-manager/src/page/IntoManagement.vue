      ...
      <el-table
        ...
        <el-table-column label="操作" width="200">
          <template slot-scope="scope">
            ...
            <el-button
              size="mini"
              @click="handleOut(scope.$index, scope.row)">出库</el-button>
            <el-button
              size="mini"
              type="danger"
...
<script>
  ...
  export default {
    ...
    methods: {
      //获取商品列表
      initData(){
        this.$https.get('/web/intocargos/list').then((data) =>{
          console.log(data);
          this.tableData = data;
      ...
      //商品状态初始化
      colorsFormat(row, column) {
        console.log(row.goodsColors)
        if (row.goodsColors == '0') {
          return '红'
        } else if (row.goodsColors == '1') {
          return '黑'
        } else if (row.goodsColors == '2') {
          return '蓝'
        }
        return '红';
      },
